...
- [x] Rolling daily log files at `%APPDATA%\ClaudeDesktopLauncher\logs\launcher-yyyy-MM-dd.log`
...
- [x] **Seed cache** - persistent `%APPDATA%\ClaudeDesktopLauncher\seed\` snapshot of a known-good Claude login state (Cookies + Local State + Preferences)
